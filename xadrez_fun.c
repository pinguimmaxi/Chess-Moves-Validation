...
char guardar_jogo(PIECE pecas[], char qj){
    ...
    for(i = 0; i < 32; i++){
        fprintf(fp, "%c;%c;%d;%d;%c/\n",
                pecas[i].square.rank, pecas[i].square.file,
...
{
	int a = identifica_peca( x1, y1, pecas );
	int b = identifica_peca( x2, y2, pecas );
	int e1 = pecas[a].state;
	int e2 = pecas[b].state;

	pecas[a].square.rank = x2;
	pecas[a].square.file = y2;
	pecas[a].state = e2;

	pecas[b].square.rank = x1;
	pecas[b].square.file = y1;
	pecas[b].state = e1;
}

void swap_e( int a, int b, char x1, char y1, char x2, char y2, int e1, int e2, PIECE pecas[])
{
	pecas[a].square.rank = x2;
	pecas[a].square.file = y2;
	pecas[a].state = e2;
			...
			for(b=0; b<32; b++)
			{
				for(c='a'; c<='h'; c++) // for para colunas
				{
					for(d='1'; d<='8'; d++) // for para linhas
					{
						...
						if( verifica_vertical_livre(pecas[b].square.rank, pecas[b].square.file, c, d, pecas)==1
		   				 && verifica_horizontal_livre(pecas[b].square.rank, pecas[b].square.file, c, d, pecas)==1
					     && verifica_diagonal_livre(pecas[b].square.rank, pecas[b].square.file, c, d, pecas)==1 )
						{
						...
						case 100: //peao branco
						if( verifica_mov_peao_branco( pecas[b].square.rank, pecas[b].square.file, c, d, pecas ) )
						{
						...
						case 300:
						if( verifica_mov_cavalo(pecas[b].square.rank, pecas[b].square.file, c, d, pecas) )
						{
							pecas[b].square.rank = c;
							pecas[b].square.file = d;
						}
						...
						case 305:
						if( verifica_mov_bispo(pecas[b].square.rank, pecas[b].square.file, c, d, pecas) )
						{
							pecas[b].square.rank = c;
							pecas[b].square.file = d;
						}
						...
						case 500:
						if( verifica_mov_torre(pecas[b].square.rank, pecas[b].square.file, c, d, pecas) )
						{
							pecas[b].square.rank = c;
							pecas[b].square.file = d;
						}
						...
						case 900:
						if( verifica_mov_rainha(pecas[b].square.rank, pecas[b].square.file, c, d, pecas) )
						{
							pecas[b].square.rank = c;
							pecas[b].square.file = d;
						}
						...
						case 2000: //rei branco
						if( verifica_mov_rei(pecas[b].square.rank, pecas[b].square.file, c, d, pecas) )
						{
							pecas[b].square.rank = c;
							pecas[b].square.file = d;
						}
						...
						if( fx2 == 0 )
							return 0;		// deixou de estar em xeque
			...
			for(b=0; b<32; b++)
			{
				for(c='a'; c<='h'; c++) // for para colunas
				{
					for(d='1'; d<='8'; d++) // for para linhas
					{
						...
						{
						case -100: //peao preto
						if( verifica_mov_peao_preto( pecas[b].square.rank, pecas[b].square.file, c, d, pecas ) )
						{
							pecas[b].square.rank = c;
							pecas[b].square.file = d;
						}
...
int dados_inicias(PIECE pecas[])
{
	...
	int a = identifica_peca( coord_ini[0], coord_ini[1], pecas );
	int b = identifica_peca( coord_fin[0], coord_fin[1], pecas );
	int e1 = pecas[identifica_peca(coord_ini[0], coord_ini[1], pecas)].state;
	int e2 = pecas[identifica_peca(coord_fin[0], coord_fin[1], pecas)].state;

	if( fx1 == 1 || fx1 == -1)
	{
		seleciona_peca(coord_ini[0], coord_ini[1], coord_fin[0], coord_fin[1], pecas);
		if( fx1 == 1 && verifica_xeque(pecas) == 1 )
			printf("Jogada Inválida. O rei branco continua em xeque! \n");
		else if( fx1 == -1 && verifica_xeque(pecas) == -1 )
			printf("Jogada Inválida. O rei preto continua em xeque! \n");

		swap_e(a, b, coord_fin[0], coord_fin[1], coord_ini[0], coord_ini[1], e1, e2, pecas);
		return 1;
	}
